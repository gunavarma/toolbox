...
export default function HomePage() {
  ...
  return (
    <div className="flex-1 w-full pb-24 space-y-16">
      {/* 1. Google-Style Search Hero Section */}
      <section className="relative w-full pt-20 pb-12 px-4 sm:px-6 flex flex-col items-center justify-center text-center">
        <div className="max-w-3xl space-y-6 z-10 w-full">
          {/* Badge */}
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full border border-primary/20 bg-primary/5 text-primary text-xs font-semibold select-none">
            <Sparkles className="h-3.5 w-3.5" /> <span>Unified Browser Workbench</span>
          </div>

          <h1 className="text-4xl sm:text-5xl md:text-6xl font-black tracking-tight text-zinc-900 dark:text-zinc-50 leading-[1.1] select-none">
          ...
          <div className="max-w-xl mx-auto pt-6 w-full relative">
            <div className="flex items-center w-full bg-bg-secondary border border-border-primary rounded-full h-13 px-5 text-zinc-500 hover:border-primary/30 focus-within:border-primary/50 focus-within:ring-2 focus-within:ring-primary/10 transition-all duration-200 shadow-sm hover:shadow-md select-none">
              <Search className="h-5 w-5 text-zinc-400 mr-3 shrink-0" />
              <input
                type="text"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                placeholder="Search 65+ offline-first tools..."
              ...
              {searchQuery ? (
                <button
                  ...
                  className="p-1 hover:bg-bg-primary rounded-full text-zinc-400 hover:text-zinc-600 transition-colors"
                >
                  <X className="h-4 w-4" />
                </button>
              ) : (
          ...
          <div className="flex flex-wrap items-center justify-center gap-x-2 gap-y-1.5 text-xs pt-2 text-zinc-400 select-none">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 w-full">
        <div className="flex flex-wrap items-center justify-center gap-2 border-b border-border-primary/40 pb-6">
          <Button
            ...
            className="rounded-full px-4 h-8.5 text-xs font-semibold"
          >
            All Utilities
          </Button>
          {categories.map((category) => {
            ...
            return (
              <Button
                ...
                className="rounded-full px-4 h-8.5 text-xs font-semibold flex items-center gap-1.5"
              >
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 w-full space-y-6">
        ...
        {filteredTools.length > 0 ? (
            ...
            {filteredTools.map((tool) => {
              ...
              return (
                <Link key={tool.id} href={`/tools/${tool.category}/${tool.slug}`} className="group">
                  <Card hoverEffect className="h-full flex flex-col justify-between p-5 min-h-[160px]">
                    <div className="space-y-4">
                      <div className="flex justify-between items-start">
                        <div className="h-10 w-10 rounded-xl bg-bg-secondary border border-border-primary/80 flex items-center justify-center text-text-muted group-hover:text-primary group-hover:border-primary/30 transition-all duration-200">
                          <Icon className="h-5 w-5" />
                        </div>
                        <span className="text-[9px] font-bold text-text-muted/80 border border-border-primary/60 px-2.5 py-0.5 rounded-full uppercase tracking-wider font-mono">
                          {categoryDetails?.name || "Utility"}
                        </span>
                      </div>

                      <div className="space-y-1">
                        <h3 className="text-sm font-bold text-text-main group-hover:text-primary transition-colors duration-200">
                          {tool.name}
                        </h3>
                    ...
                    <div className="text-[10px] font-bold text-text-muted group-hover:text-primary uppercase tracking-widest pt-4 flex items-center justify-between border-t border-border-primary/40 mt-4">
          ...
          <Card className="p-8 text-center flex flex-col items-center justify-center space-y-4 min-h-[220px]">
      ...
      <section className="max-w-3xl mx-auto px-4 sm:px-6 pt-8 space-y-6 select-none">
        <h2 className="text-xl font-bold tracking-tight text-text-main text-center flex items-center justify-center gap-2 border-t border-border-primary/40 pt-10">
          <HelpCircle className="h-5 w-5 text-primary" /> FAQ & Platform Operations
        </h2>

        <div className="space-y-3.5">
          <div className="glass p-5 rounded-2xl border border-border-primary/50 space-y-1.5">
